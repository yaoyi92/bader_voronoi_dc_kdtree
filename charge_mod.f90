...
!-----------------------------------------------------------------------------------!
  ...
  SUBROUTINE dpbc_car(ions,dr_car)
    ...
    INTEGER :: d1,d2,d3
    LOGICAL :: done
    !! YY IF FOR orthorhombic cell
    IF (ions%lattice(1,2) < 1E-8 .and. ions%lattice(1,3) < 1E-8 & 
       .and. ions%lattice(2,1) < 1E-8 .and. ions%lattice(2,3) < 1E-8 &
       .and. ions%lattice(3,1) < 1E-8 .and. ions%lattice(3,2) < 1E-8) THEN
      drt_car(1) = dr_car(1) - NINT(dr_car(1) / ions%lattice(1,1)) &
                  * ions%lattice(1,1)
      drt_car(2) = dr_car(2) - NINT(dr_car(2) / ions%lattice(2,2)) &
                  * ions%lattice(2,2)
      drt_car(3) = dr_car(3) - NINT(dr_car(3) / ions%lattice(3,3)) &
                  * ions%lattice(3,3)
      dr_car = drt_car
    ELSE
    !! YY END orthorhonbic cel
